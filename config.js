...
// Product types — drive the dashboard dropdown AND the marketplace filters
const PRODUCT_TYPES = [
  'Communication',
  'Content Creation',
...
// Log a product view (fire-and-forget). Safe for logged-out visitors.
async function logProductView(productId) {
  if (!productId) return;
  try {
    const user = await getUser();
    await sb.from('product_views').insert({ product_id: productId, viewer_id: user ? user.id : null });
    const { data: prod } = await sb.from('products').select('view_count').eq('id', productId).single();
    if (prod) await sb.from('products').update({ view_count: (prod.view_count || 0) + 1 }).eq('id', productId);
  } catch (e) { /* never block the page on analytics */ }
}

// Count views for a product in the last N days
async function countRecentViews(productId, days = 30) {
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
  const { count } = await sb.from('product_views')
...
// Is the current user following a given account?
async function isFollowing(followedId) {
  const user = await getUser();
  if (!user) return false;
  const { data } = await sb.from('follows').select('id').eq('follower_id', user.id).eq('followed_id', followedId).maybeSingle();
  return !!data;
}
...
async function toggleFollow(followedId) {
  ...
  if (user.id === followedId) { showToast("You can't follow yourself", 'error'); return false; }
  const following = await isFollowing(followedId);
  if (following) {
    await sb.from('follows').delete().eq('follower_id', user.id).eq('followed_id', followedId);
    return false;
  } else {
    await sb.from('follows').insert({ follower_id: user.id, followed_id: followedId });
    return true;
  }
}

// Upload a profile photo, return public URL
async function uploadProfilePhoto(file, userId) {
  const ext = file.name.split('.').pop();
  const path = `avatars/${userId}-${Date.now()}.${ext}`;
  const { error } = await sb.storage.from('profile-images').upload(path, file, { upsert: true });
  if (error) throw error;
  const { data: urlData } = sb.storage.from('profile-images').getPublicUrl(path);
...
function buildNav(user, profile) {
  ...
  <nav>
    ...
    <div class="nav-links">
      ...
      <a href="accounts.html">Sellers</a>
...
.nav-search input{flex:1;border:none;padding:.55rem 1rem;font-size:.88rem;outline:none;font-family:'Manrope',sans-serif;border-radius:10px 0 0 10px;}
.nav-search button{background:#38bdf8;color:#fff;border:none;padding:.55rem 1.1rem;cursor:pointer;font-weight:600;font-size:.85rem;font-family:'Manrope',sans-serif;transition:background .2s;border-radius:0 10px 10px 0;}
...
.nav-cart{font-size:.82rem;color:#fff;text-decoration:none;padding:.4rem .85rem;border:1.5px solid #334155;white-space:nowrap;transition:all .2s;border-radius:10px;}
.nav-cart:hover{border-color:#38bdf8;}
.nav-username{font-size:.82rem;font-weight:600;color:#fff;padding:.4rem .75rem;border:1.5px solid #334155;cursor:pointer;display:block;border-radius:10px;}
.nav-user-menu{position:relative;}
.nav-user-menu:hover .nav-dropdown{display:flex;}
.nav-dropdown{position:absolute;top:100%;right:0;background:#fff;border:1px solid #e2e8f0;min-width:160px;display:none;flex-direction:column;box-shadow:0 8px 24px rgba(0,0,0,.15);z-index:200;border-radius:12px;overflow:hidden;}
...
.btn-nav-ghost{font-size:.82rem;color:#94a3b8;text-decoration:none;padding:.4rem .85rem;border:1.5px solid #334155;transition:all .2s;border-radius:10px;}
.btn-nav-ghost:hover{color:#fff;}
.btn-nav-primary{font-size:.82rem;background:#38bdf8;color:#fff;text-decoration:none;padding:.4rem .9rem;font-weight:600;transition:background .2s;border-radius:10px;}
.btn-nav-primary:hover{background:#0ea5e9;}
.sv-badge{display:inline-flex;align-items:center;gap:4px;background:linear-gradient(135deg,#0284c7,#38bdf8);color:#fff;font-size:.62rem;font-weight:700;padding:3px 8px;letter-spacing:.04em;font-family:'League Spartan',sans-serif;border-radius:6px;}
